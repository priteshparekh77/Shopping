...
createDynamicHTMLTable("Desks", "select Product_ID,Price,Product_Name,Short_Descr,Dtl_Descr,Model,Weight FROM product where sub_categoryid=25 group by Product_ID  limit 5  " , $link);
createDynamicHTMLTable("Office Chairs", "select Product_ID,Price,Product_Name,Short_Descr,Dtl_Descr,Model,Weight FROM product where sub_categoryid=26 group by Product_ID  limit 5  " , $link);
createDynamicHTMLTable("BookShelves", "select Product_ID,Price,Product_Name,Short_Descr,Dtl_Descr,Model,Weight FROM product where sub_categoryid=27 group by Product_ID  limit 5  " , $link);

createDynamicHTMLTable("File Cabinets", "select Product_ID,Price,Product_Name,Short_Descr,Dtl_Descr,Model,Weight FROM product where sub_categoryid=28 group by Product_ID  limit 5  " , $link);

createDynamicHTMLTable("Tables", "select Product_ID,Price,Product_Name,Short_Descr,Dtl_Descr,Model,Weight FROM product where sub_categoryid=29 group by Product_ID  limit 5  " , $link);

createDynamicHTMLTable("Printer and Machine Stands", "select Product_ID,Price,Product_Name,Short_Descr,Dtl_Descr,Model,Weight FROM product where sub_categoryid=30 group by Product_ID  limit 5  " , $link);
mysql_close($link);
...
function createDynamicHTMLTable($table_name, $sql_query, $link)
{
    ...
    {        
       ...
       //echo "<br/><input type='submit' label='add to cart'";
        echo "<DIV align=center bgcolor=blue>\n";
